...
def register_miner():
    if orchestrator.state == "training" or orchestrator.state == "filtering":
        return jsonify({"error": "Registration closed during training and filtering"}), 403
...
def update():
    ...
    if updated_miner_id is not None and updated_miner_id not in orchestrator.blacklisted_miners:
...
def trigger_error():
    ...
    if orchestrator.trigger_error_for_miner(public_address):
        return jsonify({"message": "Error triggered successfully", "state": orchestrator.state}), 200
    else:
